...
var requestModule = require('./requestctrl')
 ...
 */
var findCallGraphObjectExpressionFunctions = function (props, current_path, filename) {
    ...
    for (var pindex = 0; pindex < props.length; pindex++) {
        ...
        if (current_prop.value && current_prop.value.type === 'FunctionExpression') {
            ...
            var n = getCallGraphFieldsForASTNode(current_prop.value);
            appendFunctionMap(key + '___' + n._id, n, filename);
            ...
            findCallGraphObjectExpressionFunctions(current_prop.value.properties, current_path, filename);
        } else {
...
function isContainProperty(functionMap, function_name) {
    for (var name in functionMap) {
        var realname = name.split('___')[0];
        if (function_name === realname)
            return functionMap[name]['node'];
    }
    return null;
}


function appendFunctionMap(functionName, functionNode, filename) {
    const blackList = ['success', 'fail', 'complete', 'error']

    let name = functionName.split('___')[0];
    if (blackList.indexOf(name) > -1)
        return;
    functionMap[functionName] = {'node': functionNode, 'filename': filename};
    functionMap_current[functionName] = {'node': functionNode, 'filename': filename};
}


var callRelations = []

function buildCBG(outputDir) {
    for (var functionName in functionMap) {
        // if(functionName.indexOf('getPhoneNumber') > -1) {
        //     console.log(functionName)
        requestModule.buildIntraDataFlowModel(functionName, functionMap[functionName].node, functionMap[functionName].filename);
        // }
    }
    // console.log(callRelations)
    requestModule.buildInterDataFlowModel(outputDir, functionMap, callRelations);
}
...
GraphBuilder.prototype.getInterProceduralModelNodesAndEdges = async function (semantic_types, options) {
    ...
    await pageScopeTrees.forEach(async function (scopeTree, pageIndex) {
        ...
        var filename = scopeTree.root.ast.value
        ...
        if (options.ipcg) {
            ...
            walkes(mainAST, {
                Property: function (node, recurse) {
                    // CASE 0: f: function(){}
                    // miniapps mainly use such pattern
                    if (node.key && node.key.name && node.value && node.value.type === 'FunctionExpression') {
                        var n = getCallGraphFieldsForASTNode(node.value);
                        appendFunctionMap(node.key.name + '___' + n._id, n, filename);
                        // console.log(functionMap[node.key.name])
                    }
                ...
                FunctionDeclaration: function (node, recurse) {
                    ...
                    if (node.id && node.id.name) {
                        var n = getCallGraphFieldsForASTNode(node);
                        appendFunctionMap(node.id.name + '___' + n._id, n, filename);
                        ...
                        if (node.body && node.body.type === 'BlockStatement') {
                            ...
                            for (var q = 0; q < body.length; q++) {
                                ...
                                if (expr && expr.type === 'ReturnStatement' && expr.argument && expr.argument.type === 'ObjectExpression') {
                                    ...
                                    findCallGraphObjectExpressionFunctions(props, '' + node.id.name + '(...)', filename); // TODO: handle better this case
                                }
                            }
                        } // END CASE 5.1
                    }
                        ...
                        for (var z = 0; z < node.declarations.length; z++) {
                            ...
                            if (variableDeclarator && variableDeclarator.type === 'VariableDeclarator' &&
                                ...
                                if (variableDeclarator.init.type === 'FunctionExpression') {
                                    var n = getCallGraphFieldsForASTNode(variableDeclarator.init);
                                    appendFunctionMap(variableDeclarator.id.name + '___' + n._id, n, filename);
                                    ...
                                    findCallGraphObjectExpressionFunctions(props, variableDeclarator.id.name, filename);
                ...
                AssignmentExpression: function (node, recurse) {
                    ...
                    if (node.right && node.right.type === 'FunctionExpression') {
                        if (node.left) {
                            if (node.left.type === 'Identifier') {
                                var n = getCallGraphFieldsForASTNode(node.right);
                                appendFunctionMap(node.left.name + '___' + n._id, n, filename);
                            }
                    ...
                    // CASE 5. obj1.obj2.dict={ k1: { f1: function(){} } } or dict={ k1: { f1: function(){} } }
                        ...
                        if (node.left && node.right && node.right.properties) {
                            if (node.left.type === 'Identifier') {
                                ...
                                findCallGraphObjectExpressionFunctions(props, node.left.name, filename);
                            }
                            else if (node.left.type === 'MemberExpression' && node.left.property && node.left.property.type === 'Identifier' && node.left.property.name) {

                                var key = getMemberExpressionAsString(node.left);
                                var props = node.right.properties;
                                findCallGraphObjectExpressionFunctions(props, key, filename);
                            }
                        }
                    }
                    ...
                    // ALIAS CASE 2: obj1.obj2.alias = func;  or obj1.obj2.alias = a.b.func;
                        ...
                        if (node.right.type === 'Identifier') {
                            ...
                            if (functionMap && isContainProperty(functionMap_current, function_actual_name) && !isContainProperty(functionMap_current, function_alias_name)) {
                                appendFunctionMap(function_alias_name + '___' + node._id, isContainProperty(functionMap_current, function_actual_name), filename)
                            } else {
        ...
        //             let cfgNode = allCFGNodes[index];
        //             if (cfgNode.astNode) {
        //                 let rawCode = escodegen.generate(cfgNode.astNode);
        //                 cfgRawNodes.push(rawCode);
        //                 // console.log(cfgNode.reachIns)
        //             }
        //         }
        ...
        if (options.erddg || options.ipcg) {
            ...
            walkes(mainAST, {
                VariableDeclarator: function (node, recurse) {
                    if (node && node.id && node.id.name && node.init) {
                        if (node.init.type && node.init.type == 'CallExpression')
                            declarations[node.id.name] = escodegen.generate(node.init);
                    }
                ...
                CallExpression: function (node, recurse) {
                    ...
                    var blackList = ['wx.login', 'swan.login', 'my.login', 'tt.login']
                    ...
                    if (blackList.indexOf(callerName) == -1 && options.ipcg && node && node.type === 'CallExpression' && node.callee) {
                        
                        if (node.callee.property)
                            callerName = escodegen.generate(node.callee.property);

                        if (functionMap && isContainProperty(functionMap, callerName)) {
                            var container = '';
                            for (var name in functionMap_current) {
                                var functionNode = functionMap_current[name]['node'];
                                var functionRange = functionNode.range, callerRange = node.range;
                                if (functionRange[0] <= callerRange[0] && functionRange[1] >= callerRange[1]) {
                                    container = name;
                                    break;
                                }
                            }
                            // console.log(container, callerName)
                            // console.log(functionMap_current[container]._id, isContainProperty(functionMap, callerName)._id)
                            if (container != '') {
                                var functionDefinitionNode = isContainProperty(functionMap, callerName);
                                var args = {};
                                ...
                                callRelations.push({
                                    fromNode: node,
                                    fromId: node._id,
                                    // fromFunction: functionMap_current[container]._id,
                                    fromFunction: container,
                                    functionName: callerName,
                                    toFunction: functionDefinitionNode._id
                                });
                                let edge = {
                                    fromId: '' + node._id,
                                    fromFunction: functionMap_current[container]['node']._id,
                            ...
                            // console.log('Function %s not found!', callerName)
                        }
                    }
    ...
    console.log('start to build CBG')
    // try {
    buildCBG(options.output);
    // } catch (err) {
    //     console.error(`Error build CBG: ${err.message} in ${options.output}`);
    // }
    console.log('CBG building finished!')

    return { 'nodes': g_nodes, 'edges': g_edges, 'functionMap': functionMap };
}

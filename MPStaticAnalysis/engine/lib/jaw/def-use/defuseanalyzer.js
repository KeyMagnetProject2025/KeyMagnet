...
DefUseAnalyzer.prototype.findDUPairs = function (model) {
    ...
    model.graph[2].forEach(function (node) {
        // console.log(escodegen.generate(node.astNode))
        ...
        nodeCUse.values().forEach(function (elem) {
            ...
            if(pairs){
                ...
                // console.log(elem.variable.name, escodegen.generate(elem.definition.fromNodeActual))
            ...
            if(node.params && node.params.length){
                for(var i=0; i< node.params.length; i++){
                    ...
                    if(param && param.name){
                        ...
                        if(!!definedVar){
                            generatedVarDef.add(varDefFactory.create(definedVar, defFactory.createLiteralDef(cfgNode, node.range, node)));
                        }
                    }
                }
            }
        ...
        AssignmentExpression: function (node, recurse) {
            ...
            if (!!definedVar) {
                if (node.right.type === 'FunctionExpression') {
                    generatedVarDef.add(varDefFactory.create(definedVar, defFactory.createFunctionDef(cfgNode, node.right.range, node)));
                } else {
                    generatedVarDef.add(varDefFactory.create(definedVar, defFactory.createLiteralDef(cfgNode, node.right.range, node)));
            ...
            if (!!definedVar) {
                if (!!node.init) {
                    if (node.init.type === 'FunctionExpression') {
                        generatedVarDef.add(varDefFactory.create(definedVar, defFactory.createFunctionDef(cfgNode, node.init.range, node)));
                    } else {
                        generatedVarDef.add(varDefFactory.create(definedVar, defFactory.createLiteralDef(cfgNode, node.init.range, node)));
                        recurse(node.init);
                    }
                } else {
                    generatedVarDef.add(varDefFactory.create(definedVar, defFactory.createUndefinedDef(cfgNode, node.range, node)));
                }
            }
        },
        SwitchCase: function () {}

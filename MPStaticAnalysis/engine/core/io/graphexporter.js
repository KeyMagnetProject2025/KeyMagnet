...
function getNodeDataForGraphML(node) {
    ...
    let type = (undefined !== node.type) ? JSON.stringify(node.type).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
    let kind = (undefined !== node.kind) ? JSON.stringify(node.kind).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
    let name = (undefined !== node.name) ? JSON.stringify(node.name).replace(/[^\\]\\"/g, (match, index) => {
            return '' + match[0] + '""'
        }) :
        (undefined !== node.test) ? JSON.stringify(node.test).replace(/[^\\]\\"/g, (match, index) => {
            return '' + match[0] + '""'
        }) : // if Statement Condition
        (undefined !== node.operator) ? JSON.stringify(node.operator).replace(/[^\\]\\"/g, (match, index) => {
            return '' + match[0] + '""'
        }) : // BinaryExpression Operator or AssignmentExpression
        (undefined !== node.init) ? '=' : ''; //  VariableDeclarator Assignment

    let range = (undefined !== node.range) ? JSON.stringify(node.range).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
    let loc = (undefined !== node.loc) ? JSON.stringify(node.loc).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
    let value = (undefined !== node.value) ? JSON.stringify(node.value).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
    let raw = (undefined !== node.raw) ? JSON.stringify(node.raw).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
    let async = (undefined !== node.async) ? JSON.stringify(node.async).replace(/[^\\]\\"/g, (match, index) => {
        return '' + match[0] + '""'
    }) : '';
...
function getRelationDataForGraphML(rel, id) {
    ...
    let fromFunction = json.stringify(rel.fromFunction)
    ...
    let relItem =
        `<edge id="${iid}" source="${fromId}" container="${fromFunction}" target="${toId}" label=${relationLabel}>
 ...
 */
GraphExporter.prototype.exportToGraphML = function (graph, graphId, outputFolder) {
    "use strict";
    const xmlFile = pathModule.join(outputFolder, constantsModule.ASTxmlFile);
    var fp = fs.openSync(xmlFile, 'w');
...
function getRelsHeaderCSVLine() {
    let d = constantsModule.outputCSVDelimiter;
    return `FromId:START_ID${d}FromFunction:Function_Name${d}ToId:END_ID${d}RelationLabel:TYPE${d}RelationType${d}Arguments\n`
}
